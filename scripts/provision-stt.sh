...
RU_EXAMPLE_SHA="2c456b64bf95d530a04ed9c1f1f50a9c22a53c2b2e27fed65d693fc54334f33f"
...
# text (verified: sha256 2c456b64…). Requires ffmpeg (present in CI).
if [[ -f "$DIR/ru_example.wav" ]]; then
    ...
    ffmpeg -y -loglevel error -ss 2.0 -i "$DIR/long_example.wav" -t 10.5 -ar 16000 -ac 1 -c:a pcm_s16le "$DIR/ru_example.wav"
    local_ru="$(sha256sum "$DIR/ru_example.wav" | awk '{print $1}')"
    if [[ "$local_ru" != "$RU_EXAMPLE_SHA" ]]; then
        echo "mismatch: $DIR/ru_example.wav expected $RU_EXAMPLE_SHA got $local_ru" >&2
        exit 1
